...
import noList from "../../common/noList";
  export default {
    ...
    methods: {
      ...
      init(info, clear){
        var defaultParmas = { page: this.page, rows: this.rows, type: this.active, cityId: this.cityId || 1 }
      ...
      getParameter(){
        this.$http.post('appServiceAccount/getGrangeParameter', { cityId: this.cityId }).then(res=>{
          if(res.msg=='success'){
            // 区域
